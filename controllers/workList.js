...
exports.getAll = (req, res, next) => {
    ...
    req.getConnection((err, connection) => {
        ...
        let sql = `SELECT * FROM work_list where td_username = '${username}' and td_status <> 9 ORDER BY td_status ASC`;
...
exports.upStatusWork = async (req, res, next) => {
    ...
    if (body.username != '' && body.td_id != '') {
        req.getConnection((err, connection) => {
            if (err) return next(err)
            result = connection.query(`UPDATE work_list SET td_upBy = '${body.username}' , td_upDt = '${date}' , td_status = '1' WHERE td_id = '${body.td_id}' `)
            console.log(result != '' ? 'Ins-success' : 'Ins-fail')
        })
        res.send({ 'status': 'ok', 'result': result })
    } else {
